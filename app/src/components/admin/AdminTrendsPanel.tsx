...
export default function AdminTrendsPanel({
  ...
  const data = useMemo(() => {
    const sliceSeries = (values: number[]) => values.slice(-range);
    return {
      revenue: sliceSeries(revenueSeries),
      orders: sliceSeries(ordersSeries),
      users: sliceSeries(usersSeries),
      dates: dates.slice(-range),
    };
  }, [range, dates, revenueSeries, ordersSeries, usersSeries]);
  const labelStep = range >= 30 ? 3 : range >= 14 ? 2 : 1;
