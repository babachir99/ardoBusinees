...
import {
  ...
  CarsExplorerSection,
  CarsMySection,
} from "@/components/cars/CarsStoreSections";
import {
  buildCarsStoreHref,
  getCarsDealersData,
  getCarsExplorerData,
...
export default async function CarsStorePage({
  locale,
  activeTab,
  searchParams,
}: {
  locale: string;
  activeTab: CarsStoreTab;
  searchParams: CarsStoreSearchParams;
}) {
  const session = await getServerSession(authOptions);
  const isFr = locale === "fr";
  const formPath = buildCarsStoreHref(locale, {
    tab: activeTab,
    includeLocale: true,
  });
  const loginHref = `/login?callbackUrl=${encodeURIComponent(`/${locale}/stores/jontaado-cars/my`)}`;

  const [explorerData, dealersData, myDashboardData] = await Promise.all([
    activeTab === "explore" ? getCarsExplorerData(searchParams) : Promise.resolve(null),
    activeTab === "dealers" ? getCarsDealersData(searchParams) : Promise.resolve(null),
    ...
    <div className="min-h-screen bg-jonta text-zinc-100">
      ...
      <main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 pb-24 pt-6 sm:px-6">
        ...
        {activeTab === "explore" && explorerData ? (
          <CarsExplorerSection locale={locale} basePath={formPath} data={explorerData} />
        ) : null}

        {activeTab === "dealers" && dealersData ? (
          <CarsDealersSection locale={locale} basePath={formPath} data={dealersData} />
        ) : null}

        {activeTab === "my" ? (
          <CarsMySection locale={locale} loginHref={loginHref} data={myDashboardData} />
        ) : null}
      </main>
      <Footer />
    </div>
