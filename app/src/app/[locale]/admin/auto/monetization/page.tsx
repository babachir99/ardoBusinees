...
export default async function AdminAutoMonetizationPage({
  params,
  searchParams,
}: {
  ...
  const purchases = await prisma.autoMonetizationPurchase.findMany({
    where,
  ...
  const t = {
    title: locale === "fr" ? "AUTO monetization ops" : "AUTO monetization ops",
    subtitle:
      locale === "fr"
        ? "Suivi des achats AUTO sur 30 jours (ou plage filtree)."
        : "AUTO purchases over the last 30 days (or filtered range).",
                    ...
                    <tr key={purchase.id} className="border-t border-white/10">
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="flex justify-end gap-2">
                          <Link
                            href={{ pathname: "/admin", query: { opsFilter: "AUTO_MONETIZATION", focus: purchase.id } }}
                            className="rounded-full border border-white/20 px-3 py-1 text-[11px] font-semibold text-white"
                          >
                            {t.openOps}
                          </Link>
                          <Link
                            href={`/admin/auto/monetization/${purchase.id}`}
