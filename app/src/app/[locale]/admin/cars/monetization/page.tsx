...
export default async function AdminCarMonetizationPage({
  params,
  searchParams,
}: {
  ...
  const purchases = await prisma.carMonetizationPurchase.findMany({
    where,
  ...
  const t = {
    title: locale === "fr" ? "CARS monetization ops" : "CARS monetization ops",
    subtitle:
      locale === "fr"
        ? "Suivi des achats CARS sur 30 jours (ou plage filtree)."
                    ...
                    <tr key={purchase.id} className="border-t border-white/10">
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="flex justify-end gap-2">
                          <Link
                            href={{ pathname: "/admin", query: { opsFilter: "CARS_MONETIZATION", focus: purchase.id } }}
                            className="rounded-full border border-white/20 px-3 py-1 text-[11px] font-semibold text-white"
                          >
                            {t.openOps}
                          </Link>
                          <Link
                            href={`/admin/cars/monetization/${purchase.id}`}
