...
export default async function AdminCarsReceiptPage({
  params,
}: {
  ...
  const purchase = await prisma.carMonetizationPurchase.findUnique({
    where: { id: purchaseId },
    select: {
      id: true,
      createdAt: true,
      ...
      <main className="mx-auto w-full max-w-3xl">
        <section className="rounded-3xl border border-white/10 bg-zinc-900/70 p-6 print:border-zinc-700 print:bg-white print:text-zinc-900">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3 print:hidden">
            <Link href="/admin/cars/monetization" className="rounded-full border border-white/20 px-4 py-2 text-xs font-semibold text-white">
          ...
          <h1 className="text-2xl font-semibold">{locale === "fr" ? "Recu monetization CARS" : "CARS monetization receipt"}</h1>
