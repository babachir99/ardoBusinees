...
export default async function AutoDealerDetailPage({
  params,
}: {
  ...
  const dealer = await prisma.autoPublisher.findFirst({
    where: {
      slug,
      status: "ACTIVE",
      type: "DEALER",
    ...
    <div className="min-h-screen bg-jonta px-6 pb-24 pt-8 text-zinc-100">
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        <section className="rounded-3xl border border-white/10 bg-gradient-to-br from-cyan-300/15 via-zinc-900 to-zinc-900 p-8">
          <Link href="/auto/dealers" className="rounded-full border border-white/20 px-3 py-1 text-xs font-semibold text-white">
            {t.back}
          </Link>
