...
async function ensureUniqueSlug(base: string) {
  const existing = await prisma.carPublisher.findMany({
    where: {
      slug: {
        startsWith: base,
      },
...
export async function GET(request: NextRequest) {
  ...
  const publishers = await prisma.carPublisher.findMany({
    where,
...
export async function POST(request: NextRequest) {
  ...
  if (!canManageCarPublisher(session.user)) {
  ...
  const baseSlug = slugifyCarPublisher(name);
  const slug = await ensureUniqueSlug(baseSlug);

  const created = await prisma.$transaction(async (tx) => {
    const publisher = await tx.carPublisher.create({
      data: {
        type: "DEALER",
        name,
        slug,
    ...
    await tx.carPublisherMember.create({
      data: {
  ...
  auditLog({
    ...
    action: "cars.publisherCreate",
    entity: { type: "car_publisher", id: created.id },
    outcome: "SUCCESS",
