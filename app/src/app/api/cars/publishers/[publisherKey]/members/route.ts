...
export async function POST(
  ...
  if (!isAdmin) {
    const ownerMembership = await prisma.carPublisherMember.findFirst({
      where: {
  ...
  try {
    const member = await prisma.carPublisherMember.create({
      data: {
        publisherId: publisher.id,
    ...
    auditLog({
      ...
      action: "cars.publisherAddMember",
      entity: { type: "car_publisher", id: publisher.id },
      outcome: "SUCCESS",
