...
function isListingKind(kind: ImmoMonetizationKind) {
...
export async function POST(request: NextRequest) {
  ...
  const userRate = checkRateLimit({
    key: `immo:monetization:checkout:user:${session.user.id}`,
    limit: 10,
    windowMs: 60_000,
  });
  ...
  const globalRate = checkRateLimit({
    key: "immo:monetization:checkout:global",
    limit: 30,
    windowMs: 60_000,
  });
  ...
  if (isListingKind(kind)) {
    ...
    if (!publisher || publisher.type !== "AGENCY" || publisher.status !== "ACTIVE") {
      return respond(errorResponse(404, "PUBLISHER_NOT_FOUND", "Active agency publisher not found."));
    }
    ...
    if (!isAdmin) {
      ...
      if (!membership) {
        return respond(errorResponse(403, "FORBIDDEN", "Only active agency members can purchase packs."));
      }
    }
  ...
  const existingPendingPurchase = await prisma.immoMonetizationPurchase.findFirst({
    where: {
  ...
  if (existingPendingPurchase?.paymentLedger?.providerIntentId) {
    ...
    auditLog({
      ...
      actor: { userId: session.user.id, role: session.user.role ?? null },
      action,
      entity: { type: "ImmoMonetizationPurchase", id: existingPendingPurchase.id },
      outcome: "SUCCESS",
      reason: AuditReason.SUCCESS,
      metadata: {
        deduped: true,
  ...
  try {
    ...
    auditLog({
      ...
      actor: { userId: session.user.id, role: session.user.role ?? null },
      action,
      entity: { type: "ImmoMonetizationPurchase", id: resolvedListing?.id ?? resolvedPublisherId },
      outcome: "ERROR",
      reason: AuditReason.DB_ERROR,
    });
    return respond(errorResponse(503, "PRISMA_ERROR", "Database unavailable."));
