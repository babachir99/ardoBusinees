...
const PRICING: Record<AutoMonetizationKind, { durationDays: number; amountCents: number; currency: string }> = {
...
function parseKind(value: unknown): AutoMonetizationKind | null {
  ...
  if (
    ...
    return normalized as AutoMonetizationKind;
  }
  return null;
}
...
function isListingKind(kind: AutoMonetizationKind) {
...
export async function POST(request: NextRequest) {
  ...
  const action = "auto.monetizationCheckoutCreate";
  ...
  if (!session?.user?.id) {
    auditLog({
      ...
      actor: { system: true },
      action,
      entity: { type: "AutoMonetizationPurchase" },
      outcome: "DENIED",
  ...
  const userRate = checkRateLimit({
    key: `auto:monetization:checkout:user:${session.user.id}`,
    limit: 10,
    windowMs: 60_000,
  });
  ...
  const globalRate = checkRateLimit({
    key: "auto:monetization:checkout:global",
    limit: 30,
    windowMs: 60_000,
  });
  ...
  if (isListingKind(kind)) {
    ...
    try {
      const creditApplied = await prisma.$transaction(async (tx) => {
        const consumed =
          kind === "FEATURED"
            ? await tx.autoMonetizationBalance.updateMany({
                where: {
                  publisherId,
        ...
        const latest = await tx.autoListing.findUnique({
          where: { id: listing.id },
          select: {
            id: true,
            status: true,
        ...
        const balance = await tx.autoMonetizationBalance.findUnique({
          where: { publisherId },
          select: {
            boostCredits: true,
        ...
        const updatedListing = await tx.autoListing.findUnique({
          where: { id: listing.id },
          select: {
            id: true,
            featuredUntil: true,
      ...
      if (creditApplied) {
        auditLog({
          ...
          actor: { userId: session.user.id, role: session.user.role ?? null },
          action,
          entity: { type: "AutoListing", id: listing.id },
          outcome: "SUCCESS",
  ...
  const existingPendingPurchase = await prisma.autoMonetizationPurchase.findFirst({
    where: {
  ...
  if (existingPendingPurchase?.paymentLedger?.providerIntentId) {
    ...
    auditLog({
      ...
      actor: { userId: session.user.id, role: session.user.role ?? null },
      action,
      entity: { type: "AutoMonetizationPurchase", id: existingPendingPurchase.id },
      outcome: "SUCCESS",
      reason: AuditReason.SUCCESS,
      metadata: {
        deduped: true,
  ...
  try {
    const created = await prisma.$transaction(async (tx) => {
      const ledger = await tx.paymentLedger.create({
        data: {
          ...
          contextType: PaymentLedgerContextType.AUTO_MONETIZATION,
      ...
      const purchase = await tx.autoMonetizationPurchase.create({
        data: {
          listingId: resolvedListing?.id ?? null,
          publisherId: resolvedPublisherId,
          kind,
    ...
    auditLog({
      ...
      actor: { userId: session.user.id, role: session.user.role ?? null },
      action,
      entity: { type: "AutoMonetizationPurchase", id: created.purchase.id },
      outcome: "SUCCESS",
      reason: AuditReason.SUCCESS,
      metadata: {
        listingId: resolvedListing?.id ?? null,
    ...
    auditLog({
      ...
      actor: { userId: session.user.id, role: session.user.role ?? null },
      action,
      entity: { type: "AutoMonetizationPurchase", id: resolvedListing?.id ?? resolvedPublisherId },
      outcome: "ERROR",
      reason: AuditReason.DB_ERROR,
    });
    return respond(errorResponse(503, "PRISMA_ERROR", "Database unavailable."));
