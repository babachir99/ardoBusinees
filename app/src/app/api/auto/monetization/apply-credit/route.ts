...
export async function POST(request: NextRequest) {
  ...
  const action = "auto.monetizationApplyCredit";
  ...
  if (!session?.user?.id) {
    auditLog({
      ...
      actor: { system: true },
      action,
      entity: { type: "AutoMonetizationBalance" },
      outcome: "DENIED",
  ...
  const rate = checkRateLimit({
    key: `auto:monetization:apply-credit:user:${session.user.id}`,
    limit: 30,
    windowMs: 60_000,
  });
  ...
  if (!isAdmin) {
    ...
    if (!membership) {
      return respond(errorResponse(403, "FORBIDDEN", "Only active dealer members can use credits."));
    }
  }
  ...
  try {
    ...
    if (!result) {
      auditLog({
        ...
        actor: { userId: session.user.id, role: session.user.role ?? null },
        action,
        entity: { type: "AutoMonetizationBalance", id: publisherId },
        outcome: "CONFLICT",
        reason: AuditReason.STATE_CONFLICT,
        metadata: { kind },
      });
